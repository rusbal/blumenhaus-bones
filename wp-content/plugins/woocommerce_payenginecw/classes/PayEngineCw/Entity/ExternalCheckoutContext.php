...
/**
 * @Entity(tableName = 'woocommerce_payenginecw_ecc')
 * @Filter(name = 'loadContextCookie', where = 'contextId = >contextId AND cookieKey = >cookieKey', orderBy = 'contextId')
 */
class PayEngineCw_Entity_ExternalCheckoutContext extends Customweb_Payment_ExternalCheckout_AbstractContext {
	...
	protected function loadPaymentMethodByMachineName($machineName){
		$paymentMethods = PayEngineCw_Util::getPaymentMethods(true);
		foreach($paymentMethods as $method) {
			$instance = PayEngineCw_Util::getPaymentMehtodInstance($method);
	 ...
	 * @return PayEngineCw_Entity_ExternalCheckoutContext
	 */
	public static function getContextById($id, $loadFromCache = true) {
		return PayEngineCw_Util::getEntityManager()->fetch('PayEngineCw_Entity_ExternalCheckoutContext', $id, $loadFromCache);
	}
	...
	public static function getReusableContextFromCookie($loadFromCache = true) {
		if (isset($_COOKIE['payenginecw-woocommerce-context-id']) && isset($_COOKIE['payenginecw-woocommerce-context-key'])) {
			try {
				$result = PayEngineCw_Util::getEntityManager()->searchByFilterName('PayEngineCw_Entity_ExternalCheckoutContext', 'loadContextCookie', array('>contextId' => $_COOKIE['payenginecw-woocommerce-context-id'], '>cookieKey' => $_COOKIE['payenginecw-woocommerce-context-key']), $loadFromCache);
				if (count($result) > 0) {
					$context = current($result);
					if ($context instanceof PayEngineCw_Entity_ExternalCheckoutContext && $context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_PENDING) {
						return $context;
					}
				}
			}
	...
	public function updateFromCart($cart) {
		...
		if (function_exists('get_woocommerce_currency')) {
			...
			$currencyCode = PayEngineCw_Util::getShopOption('____module_name_____currency');
		}
		...
		if(PayEngineCw_ConfigurationAdapter::getExternalCheckoutPlacement() == 'checkout') {
		...
		$this->setInvoiceItems(PayEngineCw_CartUtil::getInoviceItemsFromCart($cart));
