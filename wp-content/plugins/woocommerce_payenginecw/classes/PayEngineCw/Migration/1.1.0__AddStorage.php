...
class PayEngineCw_Migration_1_1_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;	

		$query = "SHOW TABLES IN `".DB_NAME."` LIKE  'payenginecw_storage';";
		...
		if(is_array($select) && count($select) > 0) {
			...
			$tableName = $entityManager->getTableNameForEntityByClassName('PayEngineCw_Entity_Storage');
			$driver->query("RENAME TABLE `payenginecw_storage` TO `" . $tableName . "`")->execute();
		}
		else {
			$driver->query(
			
				"CREATE TABLE IF NOT EXISTS `".$wpdb->prefix . "woocommerce_payenginecw_storage` (
