...
class PayEngineCw_Migration_2_0_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;
		
		$tableName = $wpdb->prefix . 'payenginecw_transactions';
		...
		$tableNameNew = $entityManager->getTableNameForEntityByClassName('PayEngineCw_Entity_Transaction');
		
		$driver->query("RENAME TABLE `" . $tableName . "` TO `" . $tableNameNew . "`")->execute();
		
		$driver->query(
				"CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "woocommerce_payenginecw_customer_contexts` ( 
