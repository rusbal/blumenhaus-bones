...
class PayEngineCw_RecurringTransactionContext extends PayEngineCw_TransactionContext implements Customweb_Payment_Authorization_Recurring_ITransactionContext
{
	protected $initialTransactionId;
	
	private $initialTransaction;
	
	public function __construct(PayEngineCw_Entity_Transaction $transaction, PayEngineCw_AbstractRecurringOrderContext $orderContext) {
		...
		if(empty($initialTransactionId)){
			$initialTransaction = PayEngineCw_Util::getAuthorizedTransactionByPostId($orderContext->getInitialOrderId());
			if(empty($initialTransaction)){
				$initialTransaction = PayEngineCw_Util::getAuthorizedTransactionByOrderId($orderContext->getInitialOrderId());
			}
	...
	public function getInitialTransaction() {
		if ($this->initialTransaction === NULL) {
			$this->initialTransaction = PayEngineCw_Util::getTransactionById($this->initialTransactionId);
		}
