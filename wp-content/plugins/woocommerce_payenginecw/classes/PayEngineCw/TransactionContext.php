...
class PayEngineCw_TransactionContext implements Customweb_Payment_Authorization_PaymentPage_ITransactionContext,
...
Customweb_Payment_Authorization_Widget_ITransactionContext
{
	...
	public function __construct(PayEngineCw_Entity_Transaction $transaction, PayEngineCw_OrderContext $orderContext, $aliasTransaction = null) {
		...
		$this->paymentCustomerContext = PayEngineCw_Util::getPaymentCustomerContext($this->userId);

		$this->databaseTransaction = $transaction;
		$this->databaseTransactionId = $transaction->getTransactionId();
		$this->notificationUrl = PayEngineCw_Util::getPluginUrl('notification');
		$this->successUrl = PayEngineCw_Util::getPluginUrl('success');
		$this->failedUrl = PayEngineCw_Util::getPluginUrl('failure');
		$this->ifameBreakoutUrl = PayEngineCw_Util::getPluginUrl('iframe', array(), 'breakOut');
	}
	...
	public function getDatabaseTransaction() {
		if ($this->databaseTransaction === NULL) {
			$this->databaseTransaction = PayEngineCw_Util::getTransactionById($this->databaseTransactionId);
		}
	...
	public function getAlias() {
		return PayEngineCw_Util::getAliasTransactionObject($this->aliasTransactionId, $this->userId);
	}

	public function getCustomParameters() {
		$params = array(
			'cwtid' => $this->getDatabaseTransaction()->getTransactionId(),
			'cwtt' => PayEngineCw_Util::computeTransactionValidateHash($this->getDatabaseTransaction())
		);

		$params = apply_filters('payenginecw_custom_parameters', $params);
		return $params;
	}
