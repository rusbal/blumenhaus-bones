...
jQuery(document).ready(function() {
	...
	jQuery('.payenginecw-transaction-table .payenginecw-more-details-button').each(function() {
		jQuery(this).click(function() {
			
			// hide all open 
			jQuery('.payenginecw-transaction-table').find('.active').removeClass('active');
			
			// Get transaction ID
			var mainRow = jQuery(this).parents('.payenginecw-main-row');
			var transactionId = mainRow.attr('id').replace('payenginecw-main_row_', '');
			
			var selector = '.payenginecw-transaction-table #payenginecw_details_row_' + transactionId;
			jQuery(selector).addClass('active');
			jQuery(mainRow).addClass('active');
		})
	});
	
	jQuery('.payenginecw-transaction-table .payenginecw-less-details-button').each(function() {
		jQuery(this).click(function() {
			// hide all open 
			jQuery('.payenginecw-transaction-table').find('.active').removeClass('active');
		})
...
(function($) {
	...
	var PayEngineCwLineItemGrid = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#payenginecw-decimal-places").val());
			this.currencyCode = $("#payenginecw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".payenginecw-line-item-grid input.line-item-quantity").each(function() {
				PayEngineCwLineItemGrid.attachListener(this);
			});
			$(".payenginecw-line-item-grid input.line-item-price-excluding").each(function() {
				PayEngineCwLineItemGrid.attachListener(this);
			});
			$(".payenginecw-line-item-grid input.line-item-price-including").each(function() {
				PayEngineCwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				PayEngineCwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.payenginecw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
