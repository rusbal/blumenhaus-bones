...
class ThemeSettings
{
    ...
    public function getSettings($fieldGroup) {
        return $this->settingFields[$fieldGroup];
    }
    ...
    */
    private function navHeader(){
        ...
        if (array_key_exists('title', $this->general)) {
            echo '<h3>'.ucfirst($this->general['title']).'</h3>';
        }else{
            echo '<h3>'.ucfirst($this->theme->get( 'Name' )).' Theme Settings</h3>';
        }
        ...
        foreach ($this->settingFields as $value) {
            if (is_array($value)) {
                foreach ($value as $val) {
                    ...
                    register_setting($this->settingsID, $val, array($this, 'sanitize'));
                }
            } else {
